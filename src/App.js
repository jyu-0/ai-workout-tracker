...
// ── LOG WORKOUT ─────────────────────────────────────────────────────────────
function LogWorkout({ onComplete }) {
  ...
  const addExercise = () => setExercises((prev) => [...prev, { id: Date.now(), name: "", sets: [{ reps: "", weight: "" }] }]);

  const addSet = (exId) => setExercises((prev) => prev.map((ex) => ex.id === exId ? { ...ex, sets: [...ex.sets, { reps: "", weight: "" }] } : ex));

  const removeExercise = (exId) => setExercises((prev) => prev.filter((ex) => ex.id !== exId));

  const updateSet = (exId, setIdx, field, val) => setExercises((prev) => prev.map((ex) => ex.id === exId ? { ...ex, sets: ex.sets.map((s, i) => i === setIdx ? { ...s, [field]: val } : s) } : ex));

  const updateExName = (exId, val) => setExercises((prev) => prev.map((ex) => ex.id === exId ? { ...ex, name: val } : ex));

  const handleSave = () => { setSaved(true); setTimeout(() => onComplete && onComplete(), 1200); };

  if (saved) return (
    <div style={{ ...styles.page, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "80vh" }}>
      <div style={{ fontSize: "48px", marginBottom: "16px" }}>💪</div>
      <div style={{ fontSize: "20px", fontWeight: "700", color: COLORS.accent }}>SESSION LOGGED</div>
      <div style={{ fontSize: "12px", color: COLORS.textMuted, marginTop: "8px" }}>Great work. Rest up.</div>
    </div>
  );
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={{ marginBottom: "16px" }}>
        ...
        <input style={styles.input} placeholder="e.g. Push Day, Leg Day..." value={workoutName} onChange={(e) => setWorkoutName(e.target.value)} />
      </div>

      {/* Muscle Groups */}
      <div style={{ marginBottom: "20px" }}>
        <span style={styles.label}>Muscles Targeted</span>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
          {MUSCLE_GROUPS.map((m) => (
            <button key={m} onClick={() => toggleMuscle(m)} style={{ ...styles.tag(muscles.includes(m) ? COLORS.accent : COLORS.textMuted), cursor: "pointer", background: muscles.includes(m) ? `${COLORS.accent}20` : "transparent", border: `1px solid ${muscles.includes(m) ? COLORS.accent : COLORS.border}`, fontFamily: "inherit" }}>
              {m}
            </button>
          ))}
        </div>
      ...
      {exercises.map((ex, exIdx) => (
        <div key={ex.id} style={{ ...styles.card, marginBottom: "10px" }}>
          <div style={{ display: "flex", gap: "8px", marginBottom: "10px", alignItems: "center" }}>
            <select style={{ ...styles.input, flex: 1 }} value={ex.name} onChange={(e) => updateExName(ex.id, e.target.value)}>
              <option value="">Select exercise...</option>
              {EXERCISES.map((e) => <option key={e} value={e}>{e}</option>)}
            </select>
            <button onClick={() => removeExercise(ex.id)} style={{ background: "none", border: "none", cursor: "pointer", color: COLORS.red, padding: "4px" }}>
              <Icon name="trash" size={16} color={COLORS.red} />
            </button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "40px 1fr 1fr", gap: "6px", marginBottom: "6px" }}>
            <div style={{ fontSize: "9px", color: COLORS.textMuted, textAlign: "center", paddingTop: "8px" }}>SET</div>
            <div style={{ fontSize: "9px", color: COLORS.textMuted, textAlign: "center" }}>REPS</div>
            <div style={{ fontSize: "9px", color: COLORS.textMuted, textAlign: "center" }}>WEIGHT (LBS)</div>
          </div>
          {ex.sets.map((set, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "40px 1fr 1fr", gap: "6px", marginBottom: "6px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", color: COLORS.accent, fontWeight: "700" }}>{i + 1}</div>
              <input type="number" style={{ ...styles.input, textAlign: "center" }} placeholder="—" value={set.reps} onChange={(e) => updateSet(ex.id, i, "reps", e.target.value)} />
              <input type="number" style={{ ...styles.input, textAlign: "center" }} placeholder="—" value={set.weight} onChange={(e) => updateSet(ex.id, i, "weight", e.target.value)} />
            </div>
          ))}
          <button onClick={() => addSet(ex.id)} style={{ ...styles.btn("outline"), width: "100%", marginTop: "6px", fontSize: "10px", padding: "7px" }}>
            + ADD SET
          </button>
        </div>
      ))}

      <button onClick={addExercise} style={{ ...styles.btn("outline"), width: "100%", marginBottom: "20px", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
        <Icon name="plus" size={14} color={COLORS.accent} /> ADD EXERCISE
      </button>

      {/* Recovery Inputs */}
      <span style={styles.label}>Recovery Factors</span>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "12px" }}>
        <div>
          <span style={{ ...styles.label, marginBottom: "4px" }}>Sleep (hrs)</span>
          <input type="number" style={styles.input} placeholder="7" value={sleep} onChange={(e) => setSleep(e.target.value)} />
        </div>
        <div>
          <span style={{ ...styles.label, marginBottom: "4px" }}>Diet Quality</span>
          <select style={styles.input} value={diet} onChange={(e) => setDiet(e.target.value)}>
      ...
      <div style={{ marginBottom: "20px" }}>
        <span style={styles.label}>Notes / How you felt</span>
        <textarea style={{ ...styles.input, minHeight: "70px", resize: "vertical" }} placeholder="Felt strong today, PR on bench..." value={notes} onChange={(e) => setNotes(e.target.value)} />
      </div>

      <button onClick={handleSave} style={{ ...styles.btn("primary"), width: "100%", padding: "16px", fontSize: "13px" }}>
        FINISH & LOG SESSION
      </button>
    </div>
  );
